...
unsigned long sendInterval = 5000; // Intervalo de envío en milisegundos
...
void setup() {
  ...
  SCmd.addCommand("start_sensor", start_sensor);
...
void loop() {
  ...
  if (sensorActive && (millis() - lastSendTime >= sendInterval)) {
    ...
    // Genera datos aleatorios y envíalos
    int randomValue = random(0, 255);
    char data[2];
    sprintf(data, "%02X", randomValue); // Convierte el valor a hexadecimal

    // Usa una de las funciones de transmisión
    int state = radio.transmit(data);
    if (state == RADIOLIB_ERR_NONE) {
      Serial.println("Random data sent successfully!");
    } else {
      Serial.println("Failed to send random data.");
    }
  }
